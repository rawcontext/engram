...
async function main() {
    ...
    const userPayload = createEvent({ role: "user", content: "Explain how the bitemporal memory graph handles retroactive updates." });
    await producer.send({
        topic: "raw_events",
        messages: [{ key: userPayload.event_id, value: JSON.stringify(RawStreamEventSchema.parse(userPayload)) }]
    });
    ...
    await new Promise(r => setTimeout(r, 2000));
        ...
        for (const thought of thoughts) {
            ...
            await producer.send({
                topic: "raw_events",
                messages: [{ key: thoughtPayload.event_id, value: JSON.stringify(RawStreamEventSchema.parse(thoughtPayload)) }]
            });
            console.log(`   -> ${thought}`);
            await new Promise(r => setTimeout(r, 2000)); 
        }
