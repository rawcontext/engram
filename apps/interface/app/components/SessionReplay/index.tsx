...
function isThinkingContent(content: string): boolean {
    const thinkingPatterns = [
        /^<thought>/i, /^<thinking>/i, /^<reasoning>/i,
        /analyzing/i, /checking/i, /scanning/i, /querying/i,
        /simulating/i, /correlating/i, /detecting/i,
        /resolving/i, /validating/i, /generating/i,
    ];
...
function consolidateTimeline(timeline: TimelineEvent[]): ConsolidatedMessage[] {
    ...
    for (const event of timeline) {
        ...
        const isToken = typeof content === 'string' &&
            content.length < 50 &&
            !content.includes('\n') &&
            type.includes('thought');

        const isNewThinkingBlock = typeof content === 'string' &&
            (content.startsWith('<thought>') ||
             content.startsWith('<thinking>') ||
             content.includes('...'));

        const isCompleteThought = typeof content === 'string' &&
            content.length > 50 &&
        ...
        if (isCompleteThought || isNewThinkingBlock) {
            ...
            messages.push({
                ...
                type: type.includes('action') ? 'action' :
                      type.includes('observation') ? 'observation' :
                      type.includes('system') ? 'system' : 'thought',
...
function ResponseCard({
    ...
    return (
        <div
            style={{
                position: 'relative',
                background: 'linear-gradient(135deg, rgba(226, 232, 240, 0.02) 0%, rgba(148, 163, 184, 0.04) 50%, rgba(226, 232, 240, 0.02) 100%)',
...
function QueryCard({ content }: { content: string }) {
    return (
        <div style={{
            position: 'relative',
            padding: '16px 18px',
            background: 'linear-gradient(135deg, rgba(148, 163, 184, 0.04) 0%, rgba(100, 116, 139, 0.06) 100%)',
            borderRadius: '10px',
            border: '1px solid rgba(148, 163, 184, 0.15)',
...
function StatsHeader({ messages }: { messages: ConsolidatedMessage[] }) {
    ...
    const stats = [
        { label: 'REASONING', value: reasoningCount, color: 'rgb(251, 191, 36)', glowColor: 'rgba(251, 191, 36, 0.5)' },
        { label: 'OUTPUT', value: responseCount, color: 'rgb(226, 232, 240)', glowColor: 'rgba(226, 232, 240, 0.3)' },
        { label: 'TOKENS', value: totalTokens, color: 'rgba(148, 163, 184, 0.9)', glowColor: 'rgba(148, 163, 184, 0.3)' },
    ];
    ...
    return (
        <div style={{
            ...
            background: 'linear-gradient(180deg, rgba(10, 15, 25, 0.95) 0%, rgba(15, 20, 30, 0.8) 100%)',
        }}>
            {stats.map((stat, i) => (
                <div
                    key={stat.label}
            ...
            >
                <div style={{
                    ...
                    <div style={{
                        ...
                        bottom: '40px',
                        width: '2px',
                        background: 'linear-gradient(180deg, rgba(148, 163, 184, 0.2) 0%, rgba(251, 191, 36, 0.35) 30%, rgba(251, 191, 36, 0.35) 70%, rgba(226, 232, 240, 0.25) 100%)',
                        borderRadius: '1px',
                        zIndex: 0,
                    }} />
