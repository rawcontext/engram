 ...
 */
export function useSessionStream({ sessionId, onLineageUpdate, onReplayUpdate }: UseSessionStreamOptions) {
    ...
    try {
      ...
      ws.onclose = (event) => {
        ...
        if (reconnectAttempts.current < maxReconnectAttempts) {
          ...
          console.log(`[SessionStream] Reconnecting in ${delay}ms (attempt ${reconnectAttempts.current})`);
