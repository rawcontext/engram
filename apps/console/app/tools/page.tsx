...
function highlightCypher(code: string): string {
	...
	return code
		.replace(strings, '<span class="text-[rgb(var(--console-green))]">$1</span>')
		.replace(keywords, '<span class="text-[rgb(var(--console-purple))] font-semibold">$1</span>')
		.replace(functions, '<span class="text-[rgb(var(--console-cyan))]">$1</span>')
		.replace(variables, '<span class="text-[rgb(var(--console-amber))]">$1</span>')
		.replace(numbers, '<span class="text-[rgb(var(--console-amber))]">$1</span>');
}
		...
		<div className="relative">
			...
			{isOpen && (
				<>
					...
					<div className="absolute left-0 right-0 top-full mt-2 z-20 py-1 rounded-lg bg-[rgb(var(--console-panel))] border border-[rgba(var(--console-cyan),0.2)] shadow-xl shadow-black/30">
						{options.map((option) => (
							<button
								type="button"
								key={option.value}
...
function ConfirmDialog({
	...
	const variantColor = variant === "danger" ? "--console-red" : "--console-amber";
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center">
			...
			<div className="relative panel p-6 max-w-md w-full mx-4 animate-fade-in">
				<div
					...
					style={{ background: `rgba(var(${variantColor}), 0.15)` }}
				>
					<AlertTriangle className="w-6 h-6" style={{ color: `rgb(var(${variantColor}))` }} />
				</div>
				<h3 className="font-display text-lg text-[rgb(var(--text-primary))] text-center mb-2">
					{title}
				</h3>
				<p className="text-sm text-[rgb(var(--text-secondary))] text-center mb-6">{message}</p>
				<div className="flex gap-3">
					<button
						type="button"
						onClick={onCancel}
						className="flex-1 px-4 py-2.5 rounded-lg bg-[rgb(var(--console-surface))] border border-[rgba(var(--console-cyan),0.15)] text-[rgb(var(--text-secondary))] hover:text-[rgb(var(--text-primary))] hover:border-[rgba(var(--console-cyan),0.3)] transition-colors font-mono text-sm"
					>
						Cancel
					</button>
					<button
						...
						style={{
							background: `rgb(var(${variantColor}))`,
							color: "rgb(var(--console-void))",
						}}
					>
						{confirmLabel}
					</button>
		...
		<div className="panel p-6 flex flex-col h-full">
			...
			<div className="flex items-center justify-between mb-4">
				<div className="flex items-center gap-3">
					<div className="w-10 h-10 rounded-lg bg-[rgba(var(--console-purple),0.15)] flex items-center justify-center">
						<Database className="w-5 h-5 text-[rgb(var(--console-purple))]" />
					</div>
					<div>
			...
			<div className="flex flex-wrap gap-2 mb-4">
				{EXAMPLE_QUERIES.map((example) => (
					<button
						...
						className="px-3 py-1.5 rounded-full text-xs font-mono bg-[rgb(var(--console-surface))] text-[rgb(var(--text-secondary))] hover:text-[rgb(var(--console-cyan))] hover:bg-[rgba(var(--console-cyan),0.1)] transition-colors border border-transparent hover:border-[rgba(var(--console-cyan),0.2)]"
					>
						{example.name}
					</button>
				))}
			...
			<div className="relative flex-shrink-0 mb-4">
				<div className="absolute top-3 left-3 flex items-center gap-2 text-xs font-mono text-[rgb(var(--text-muted))] z-10">
					<span className="px-2 py-0.5 rounded bg-[rgba(var(--console-purple),0.2)] text-[rgb(var(--console-purple))]">
						CYPHER
					</span>
				</div>
				<div className="relative rounded-lg bg-[rgb(var(--console-surface))] border border-[rgba(var(--console-cyan),0.1)] overflow-hidden">
					...
					<textarea
						...
						className="w-full h-48 pt-10 px-4 pb-4 bg-transparent font-mono text-sm leading-relaxed resize-none focus:outline-none text-transparent caret-[rgb(var(--console-cyan))]"
						spellCheck={false}
					/>
				</div>
			</div>
			...
			<button
				...
				className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-gradient-to-r from-[rgb(var(--console-purple))] to-[rgb(var(--console-cyan))] text-[rgb(var(--console-void))] font-mono text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-lg hover:shadow-[rgba(var(--console-purple),0.3)] transition-all mb-4"
			>
				{isExecuting ? (
					<>
			...
			{/* Error Display */}
			{error && (
				<div className="flex items-start gap-3 p-4 rounded-lg bg-[rgba(var(--console-red),0.1)] border border-[rgba(var(--console-red),0.2)] mb-4">
					<XCircle className="w-5 h-5 text-[rgb(var(--console-red))] flex-shrink-0 mt-0.5" />
					<div className="text-sm text-[rgb(var(--console-red))] font-mono">{error}</div>
				</div>
			)}
			...
			{result && (
				<div className="flex-1 min-h-0 flex flex-col">
					{/* Stats bar */}
					<div className="flex items-center gap-4 mb-3 text-xs font-mono text-[rgb(var(--text-muted))]">
						<div className="flex items-center gap-1.5">
							<Clock className="w-3.5 h-3.5 text-[rgb(var(--console-green))]" />
							<span>{result.executionTime.toFixed(1)}ms</span>
						</div>
						<div className="flex items-center gap-1.5">
							<Database className="w-3.5 h-3.5 text-[rgb(var(--console-cyan))]" />
							<span>{result.results.length} rows</span>
						</div>
					</div>

					{/* Results table */}
					<div className="flex-1 overflow-auto rounded-lg bg-[rgb(var(--console-surface))] border border-[rgba(var(--console-cyan),0.1)]">
						{result.results.length > 0 && (
							<table className="w-full text-sm font-mono">
								<thead>
									<tr className="border-b border-[rgba(var(--console-cyan),0.1)]">
										{Object.keys(result.results[0]).map((key) => (
											<th
												key={key}
												className="px-4 py-3 text-left text-xs font-semibold text-[rgb(var(--console-cyan))] uppercase tracking-wider"
											>
												{key}
											</th>
										))}
								...
								<tbody>
									{result.results.map((row, i) => (
										<tr
											...
											className="border-b border-[rgba(var(--console-cyan),0.05)] hover:bg-[rgba(var(--console-cyan),0.03)]"
										>
											{Object.values(row).map((value, j) => (
												<td
													key={`cell-${Object.keys(row)[j]}`}
													className="px-4 py-3 text-[rgb(var(--text-secondary))]"
												>
...
function VectorSearchPanel() {
	...
	const getScoreColor = (score: number) => {
		if (score >= 0.9) return "--console-green";
		if (score >= 0.7) return "--console-cyan";
		if (score >= 0.5) return "--console-amber";
		return "--console-red";
	};
	...
	return (
		<div className="panel p-6 flex flex-col h-full">
			...
			<div className="flex items-center justify-between mb-4">
				<div className="flex items-center gap-3">
					<div className="w-10 h-10 rounded-lg bg-[rgba(var(--console-cyan),0.15)] flex items-center justify-center">
						<Search className="w-5 h-5 text-[rgb(var(--console-cyan))]" />
					</div>
					<div>
						<h3 className="font-display text-base text-[rgb(var(--text-primary))]">
				...
				{(isSearching || elapsedTime > 0) && (
					<div
						className={`flex items-center gap-2 px-3 py-1.5 rounded-full font-mono text-xs ${
							isSearching
								? "bg-[rgba(var(--console-cyan),0.1)] text-[rgb(var(--console-cyan))]"
			...
			<div className="relative mb-4">
				<input
					...
					className="w-full px-4 py-3 pl-11 rounded-lg bg-[rgb(var(--console-surface))] border border-[rgba(var(--console-cyan),0.15)] focus:border-[rgba(var(--console-cyan),0.4)] focus:outline-none font-mono text-sm text-[rgb(var(--text-primary))] placeholder:text-[rgb(var(--text-dim))] transition-colors"
				/>
			...
			<div className="mb-4">
				<div className="flex items-center justify-between mb-2">
					...
					<span className="font-mono text-sm text-[rgb(var(--console-cyan))]">{limit}</span>
				</div>
				<input
					type="range"
					min={5}
					...
					className="w-full h-2 rounded-full appearance-none bg-[rgb(var(--console-surface))] cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-[rgb(var(--console-cyan))] [&::-webkit-slider-thumb]:shadow-lg [&::-webkit-slider-thumb]:shadow-[rgba(var(--console-cyan),0.3)]"
				/>
				<div className="flex justify-between text-xs font-mono text-[rgb(var(--text-muted))] mt-1">
					<span>5</span>
					<span>50</span>
				</div>
			</div>
			...
			<button
				...
				className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-gradient-to-r from-[rgb(var(--console-cyan))] to-[rgb(var(--console-blue))] text-[rgb(var(--console-void))] font-mono text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-lg hover:shadow-[rgba(var(--console-cyan),0.3)] transition-all mb-4"
			>
				{isSearching ? (
					<>
						<Loader2 className="w-4 h-4 animate-spin" />
			...
			{result && (
				<div className="flex-1 min-h-0 overflow-auto space-y-3">
					{result.results.map((item, idx) => (
						<div
							key={item.id}
							className="p-4 rounded-lg bg-[rgb(var(--console-surface))] border border-[rgba(var(--console-cyan),0.08)] hover:border-[rgba(var(--console-cyan),0.2)] transition-colors group"
						...
						>
							<div className="flex items-start justify-between gap-3 mb-2">
								...
								<div
									...
									style={{
										background: `rgba(var(${getScoreColor(item.score)}), 0.15)`,
										color: `rgb(var(${getScoreColor(item.score)}))`,
									}}
								>
								...
								{item.type && (
									<div
										...
										style={{
											background: `rgba(var(${getTypeColor(item.type)}), 0.1)`,
											color: `rgb(var(${getTypeColor(item.type)}))`,
										}}
									>
										{item.type}
									</div>
							...
							{/* ID */}
							<div className="mt-2 text-xs font-mono text-[rgb(var(--text-dim))] opacity-0 group-hover:opacity-100 transition-opacity">
								{item.id}
							</div>
						</div>
					))}
	...
	return (
		<div className="panel p-6 flex flex-col h-full">
			...
			<div className="flex items-center gap-3 mb-6">
				<div className="w-10 h-10 rounded-lg bg-[rgba(var(--console-amber),0.15)] flex items-center justify-center">
					<Wrench className="w-5 h-5 text-[rgb(var(--console-amber))]" />
				</div>
				<div>
					<h3 className="font-display text-base text-[rgb(var(--text-primary))]">Cache Controls</h3>
					<p className="text-xs text-[rgb(var(--text-muted))] font-mono">
			...
			<div className="space-y-4 flex-1">
				...
				<div className="p-4 rounded-lg bg-[rgb(var(--console-surface))] border border-[rgba(var(--console-cyan),0.08)]">
					<div className="flex items-center justify-between mb-3">
						<div className="flex items-center gap-3">
							<div className="w-8 h-8 rounded-lg bg-[rgba(var(--console-purple),0.1)] flex items-center justify-center">
								<Sparkles className="w-4 h-4 text-[rgb(var(--console-purple))]" />
							</div>
							<div>
								...
								<div className="text-xs text-[rgb(var(--text-muted))]">
									Clear cached vector embeddings
								</div>
							</div>
						</div>
						<button
							...
							className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[rgba(var(--console-red),0.1)] text-[rgb(var(--console-red))] hover:bg-[rgba(var(--console-red),0.2)] transition-colors font-mono text-xs font-medium disabled:opacity-50"
						>
					...
					<div className="flex items-center gap-2 text-xs font-mono">
						{actions.embedding.result?.success ? (
							...
							<Clock className="w-3.5 h-3.5 text-[rgb(var(--text-dim))]" />
						)}
				...
				<div className="p-4 rounded-lg bg-[rgb(var(--console-surface))] border border-[rgba(var(--console-cyan),0.08)]">
					<div className="flex items-center justify-between mb-3">
						<div className="flex items-center gap-3">
							<div className="w-8 h-8 rounded-lg bg-[rgba(var(--console-cyan),0.1)] flex items-center justify-center">
								<Database className="w-4 h-4 text-[rgb(var(--console-cyan))]" />
							</div>
							<div>
								<div className="font-mono text-sm text-[rgb(var(--text-primary))]">Query Cache</div>
								<div className="text-xs text-[rgb(var(--text-muted))]">
									Clear cached Cypher results
								</div>
							</div>
						</div>
						<button
				...
				<div className="p-4 rounded-lg bg-[rgb(var(--console-surface))] border border-[rgba(var(--console-cyan),0.08)]">
					<div className="flex items-center justify-between mb-3">
						<div className="flex items-center gap-3">
							<div className="w-8 h-8 rounded-lg bg-[rgba(var(--console-amber),0.1)] flex items-center justify-center">
								<Server className="w-4 h-4 text-[rgb(var(--console-amber))]" />
							</div>
							<div>
								<div className="font-mono text-sm text-[rgb(var(--text-primary))]">
									NATS Consumer
								...
								<div className="text-xs text-[rgb(var(--text-muted))]">
									Reset stream consumer position
								</div>
							</div>
						</div>
						<button
							...
							className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[rgba(var(--console-amber),0.1)] text-[rgb(var(--console-amber))] hover:bg-[rgba(var(--console-amber),0.2)] transition-colors font-mono text-xs font-medium disabled:opacity-50"
						>
					...
					<div className="mb-3">
						...
						<select
							...
							className="w-full px-3 py-2 rounded-lg bg-[rgb(var(--console-panel))] border border-[rgba(var(--console-cyan),0.15)] focus:border-[rgba(var(--console-cyan),0.4)] focus:outline-none font-mono text-sm text-[rgb(var(--text-primary))] appearance-none cursor-pointer"
						>
							{streams.map((stream) => (
			...
			{/* Stream Stats */}
			{streams.length > 0 && (
				<div className="mt-4 pt-4 border-t border-[rgba(var(--console-cyan),0.1)]">
							...
							<div
								...
								className={`p-3 rounded-lg text-center transition-colors ${
									stream.name === selectedStream
										? "bg-[rgba(var(--console-amber),0.1)] border border-[rgba(var(--console-amber),0.2)]"
										: "bg-[rgb(var(--console-panel))]"
								}`}
							>
								<div className="font-mono text-lg font-semibold text-[rgb(var(--text-primary))]">
		...
		<div className="space-y-6">
			...
			<div>
				<h1 className="font-display text-2xl text-[rgb(var(--text-primary))] flex items-center gap-3">
					<div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[rgb(var(--console-amber))] to-[rgb(var(--console-red))] flex items-center justify-center">
						<Wrench className="w-4 h-4 text-[rgb(var(--console-void))]" />
					</div>
					Admin Tools
				</h1>
