...
export class Executor {
  constructor(private config: WassetteConfig, private policy: SecurityPolicy) {
      enforcePolicy(config, policy);
  }

  async execute(wasmModule: WebAssembly.Module, args: string[] = []): Promise<{ stdout: string; stderr: string; exitCode: number }> {
    // Capture buffers
    let stdout = '';
    let stderr = '';
